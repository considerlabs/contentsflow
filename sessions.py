...
from agent import generate_topic_candidates, run_pipeline, load_knowledge
import notion
...
async def create_session(body: SessionCreate, db: AsyncSession = Depends(get_db)):
    ...
    kw_result = await db.execute(
        ...
        .where(Keyword.user_id == uuid.UUID(body.user_id), Keyword.is_active == True)
    )
...
async def generate_drafts(
    ...
    result  = await db.execute(select(ContentSession).where(ContentSession.id == uuid.UUID(session_id)))
...
async def _run_generation(
    ...
    input_emotion: str, input_memo: str
):
    ...
    async with AsyncSessionLocal() as db:
        ...
        kw_result = await db.execute(
            select(Keyword.keyword)
            .where(Keyword.user_id == uuid.UUID(user_id), Keyword.is_active == True)
        )
        keywords = [row[0] for row in kw_result.fetchall()]
...
async def get_session(session_id: str, db: AsyncSession = Depends(get_db)):
    result  = await db.execute(
        ...
        .where(ContentSession.id == uuid.UUID(session_id))
    )
    session = result.scalar_one_or_none()
    if not session:
        raise HTTPException(status_code=404, detail="세션을 찾을 수 없습니다.")
